...
--
-- SQLite 방언. PostgreSQL 판은 db/schema_pg.sql — **컬럼 구성이 같아야 한다.**
-- 차이와 그 이유는 schema_pg.sql 머리말에 정리돼 있다.
--
-- 시각 컬럼에 DEFAULT 를 두지 않는다. SQLite 의 datetime('now') 는
-- '2026-08-16 15:07:43'(UTC·공백·오프셋없음) 을 만들어 Python 의
-- collector/db/now() 형식('...T...+09:00') 과 어긋난다 — 한 DB 안에 두 형식이 섞였다.
-- 시각 형식은 now() 한 곳에서만 정한다.
