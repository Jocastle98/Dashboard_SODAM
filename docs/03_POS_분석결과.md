...
> 📌 아래 수치는 **예시값**이다. 저장소가 public 이므로 실제 매출을 적지 않는다.
> 검증의 논지(합계가 일치한다 / 소급이 가능하다)는 그대로 성립한다.
> 실제 값은 `python -m collector.main --status` 로 확인한다.

실측 구조 (수치는 예시):
```
합계: 총매출 1,000,000 / 할인 0 / 순매출 1,000,000
      현금 100,000 / 카드 900,000 / 부가세 90,909
주문 20건
```
확인한 사실: 합계 행이 `No.=0` 으로 맨 앞에 오고, `순매출 = 총매출 − 할인` 이며
`현금 + 카드 + …` 가 총매출과 일치한다.
...
| 4 | `총매출액` | `900,000` (예시) | `menu_sales.sales_amount` |
| 5 | `일평균매출액` | `900,000` (예시) | (파생값 — 저장 불필요) |
| 6 | `판매수량` | `50` (예시) | `menu_sales.quantity` |
...
| 대상일 | 결과 | 합계 매출 |
|---|---|---|
| 당일 | ✅ 주문 조회됨 | (값 확인 — 문서에는 적지 않는다) |
| 1년 전 같은 날 | ✅ 주문 조회됨 | (동일) |
| 2년 전 같은 날 | ✅ 주문 조회됨 | (동일) |

세 시점 모두 정상 응답했다. 확인하려던 것은 **금액이 아니라 소급 가능 여부**다.
